import { RNG, Map, FOV } from "rot-js";
...
export class GameMap {
    ...
    createMonsters(rooms) {
        ...
        for (let i = 1; i < rooms.length; i++) {
            let rng = RNG.getUniform();
            if (rng > 0.8) {
                enemy = new Entity(rooms[i]._x1 + 1, rooms[i]._y1 + 1, 'o', 'green', 'Orc');
                enemy.components.fighter = new Fighter(3, 2, 5);
            } else {
                enemy = new Entity(rooms[i]._x1 + 1, rooms[i]._y1 + 1, 'k', 'lightgreen', 'Kobold');
                enemy.components.fighter = new Fighter(1, 1, 3);
            }
    ...
    createItems(rooms) {
        ...
        healthPotion.effects = new Heal(6)
        healthPotion.blocking = false;
        this.entities.push(healthPotion)
        for (let i = 0; i < rooms.length; i++) {
            let rng = RNG.getUniform();
            if (rng > 0.6) {
                let healthPotion = new Entity(rooms[i]._x1 + 2, rooms[i]._y1 + 2, 'p', 'pink', 'Health Potion');
                healthPotion.effects = new Heal(6)
                healthPotion.blocking = false;
                this.entities.push(healthPotion)
            }
